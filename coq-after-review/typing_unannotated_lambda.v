...
This file contains type-safety \m calculus.

-> Corresponds to section 3 in paper.
-> Contains type-safety proofs.
...
exp corresponds to expressions in Figure 4.
...
value corresponds to values in Figure 5.
...
Inductive value : exp -> Prop :=    (* defn value *)
 | val_int : forall i5,
     value (e_lit i5)
 | val_abs : forall (e:exp) (A B:typ),
     lc_exp (e_ann (e_abs e) (t_arrow A B)) ->
     value (e_ann (e_abs e) (t_arrow A B))
  | val_merg : forall v1 v2,
    value v1 ->
    value v2 ->
    value (e_merg v1 v2).
...
(* Inductive wvalue : exp -> Prop :=    (* defn value *)
 | wval_int : forall i5,
     wvalue (e_lit i5)
 | wval_abs : forall (e:exp) (A B:typ),
     lc_exp (e_ann (e_abs e) (t_arrow A B)) ->
     wvalue (e_ann (e_abs e) (t_arrow A B))
  | wval_merg : forall v1 v2,
    wvalue v1 ->
    wvalue v2 ->
    wvalue (e_merg v1 v2)
 | wval_abs : forall (e:exp) (A B:typ),
...
ptyp corresponds to `Dynamic Type` shown in
Figure 7 in paper.
...
Inductive ptyp : exp -> typ -> Prop :=    (* defn pexpr *)
 ...
 | ptyp_abs : forall (e:exp) (A B:typ),
     lc_exp (e_ann (e_abs e) (t_arrow A B)) ->
     ptyp (e_ann (e_abs e) (t_arrow A B)) (t_arrow A B)
...
(******************************)
(*** Type casting relation ****)
(******************************)

(*
tred corresponds to type casting in Figure 5.
*)
...
Inductive tred : exp -> typ -> exp -> Prop := (*typed red*)
 ...
 | tred_arrow : forall e A1 B1 A2 B2,
    lc_exp (e_ann (e_abs e) (t_arrow A1 B1)) ->
    (t_arrow A1 B1) <: (t_arrow A2 B2) ->
    tred (e_ann (e_abs e) (t_arrow A1 B1)) (t_arrow A2 B2) (e_ann (e_abs e) (t_arrow A1 B1))
...
getInType corresponds to `Input Type` relation
in Figure 7 in paper.

*)

Inductive getInType : exp -> typ -> Prop :=
  | intyp_abs : forall e A B,
      getInType (e_ann (e_abs e) (t_arrow A B)) A
...
dynSemantics corresponds to `Applicative dispatch`
shown in Figure 7 in paper.
...
typing correspond to type system shown in
Figure 4 in paper.
...
step corresponds to operational semantics shown
in Figure 6 in paper.
...
Inductive step : exp -> exp -> Prop :=    (* defn step *)
 ...
 | step_beta : forall (e:exp) (A1 B1:typ) (p p':exp) (C:typ),
     ...
     value p ->
     tred p A1 p' ->
     e_app (e_ann (e_abs e) (t_arrow A1 B1)) p ~~> (open_exp_wrt_exp e p')
 ...
 (* | step_unabs : forall e A B,
    (* typing [] (e_abs e) (t_arrow A B) -> *)
    step (e_abs e) (e_ann (e_abs e) (t_arrow A B)) *)
 | step_unabs_inter : forall e A B,
    step (e_ann (e_abs e) (t_and A B)) (e_merg (e_ann (e_abs e) A) (e_ann (e_abs e) B))
 | step_beta_unabs : forall (e:exp) (A1 B1:typ) (p:exp),
     lc_exp (e_ann (e_abs e) (t_arrow A1 B1)) ->
     lc_exp (e_abs p) ->     
     e_app (e_ann (e_abs e) (t_arrow A1 B1)) (e_abs p) ~~> e_app (e_ann (e_abs e) (t_arrow A1 B1)) (e_ann (e_abs p) A1)
...
Lemma tred_value : forall v A v',
value v -> tred v A v' -> value v'.
Proof.
  intros.
  induction H0; eauto.
  - inverts* H.
...
Proof.
 ...
 - forwards* [Typ3 Sub1]: IHTyp1.
   forwards* [Typ4 Sub2]: IHTyp2.
Qed.
...
Proof.
    ...
    - exists* A B.
    - forwards* [A1 [B1 Sub1]]: IHTyp1.
      forwards* [A2 [B2 Sub2]]: IHTyp2.
      exists (t_union A1 A2) (t_and B1 B2).
      apply s_anda.
      admit.
Admitted.
...
Lemma inv_abs_sub_unann1 : forall G e A1 A2,
    typing G (e_abs e) (t_arrow A1 A2) ->
           (exists L, forall x , x \notin  L -> typing ((x, A1) :: G) (e open_ee_var x) A2).
Proof.
  (* introv Typ.
  inductions Typ; eauto.
  - forwards*: IHTyp.
...
Lemma inv_abs_sub_unann : forall G e A1 A2,
    typing G (e_abs e) (t_arrow A1 A2) ->
           (exists L, forall x , x \notin  L -> typing ((x, A1) :: G) (e open_ee_var x) A2).
Proof.
  introv Typ.
  inductions Typ; eauto; intros.
  (* - forwards*: IHTyp.
    eapply sub_transitivity; eauto.
  (* - assert (HS: subtyping B (t_arrow B1 B2)) by applys sub_transitivity H Sub.
    forwards* (?&?): IHTyp HS. *)
...
Lemma inv_abs_sub : forall G e D1 D2 A B1 B2,
    typing G (e_ann (e_abs e) (t_arrow D1 D2)) A ->
...
Proof.
  ...
  inductions Typ; eauto.
  - split*. inverts* Sub.
    forwards*: inv_abs_sub_unann.
  - assert (HS: subtyping B (t_arrow B1 B2)) by applys sub_transitivity H Sub.
    forwards* (?&?): IHTyp HS.
  - specialize (IHTyp1 e D1 D2).
    specialize (IHTyp2 e D1 D2).
    inverts* Sub.
    forwards*: IHTyp1.
    forwards*: IHTyp2.
Qed.
...
Proof.
  (* introv Typ.
  apply inv_anno in Typ.
  destruct Typ as [Typ Sub]. split*.
  inductions Typ. admit.
  inductions Typ; eauto.
...
Lemma inv_abs_sub'' : forall G e D1 D2 A,
    typing G (e_ann (e_abs e) (t_arrow D1 D2)) A ->
    typing G (e_ann (e_abs e) (t_arrow D1 D2)) (t_arrow D1 D2)
           /\ subtyping (t_arrow D1 D2) A.
Proof.
  introv Typ.
  inductions Typ; eauto.
...
(* Lemma inv_arrow : forall G e D1 D2 A1 A2,
    typing G (e_ann (e_abs e) (t_arrow D1 D2)) (t_arrow A1 A2) ->
    (exists L, forall x , x \notin  L -> typing ((x, D1)::G) (e open_ee_var x) D2)
...
Lemma inv_abs_union : forall G e D1 D2 A A1 A2,
    ...
    typing G (e_ann (e_abs e) (t_arrow D1 D2)) A1 \/ typing G (e_ann (e_abs e) (t_arrow D1 D2)) A2.
Proof.
  introv Typ Sub.
...
Proof.
  ...
  - forwards*: IHTyp.
    ...
    eapply sub_transitivity; eauto.
  - exists* A B.
  - forwards* [B1 [B2 [Typ11 [Typ12 Sub1]]]]: IHTyp1.
...
Lemma exists_ptype_pexpr : forall v,
  value v -> exists A, ptyp v A.
Proof.
  induction 1; eauto.
  destruct IHvalue1 as [A1 H1].
  destruct IHvalue2 as [A2 H2].
  exists*.
Qed.

(*

tred_preservation corresponds to Theorem 1
(Casting Preservation) in paper.

*)

Lemma tred_preservation : forall w A w', 
value w -> tred w A w' -> 
...
Proof.
  ...
  - (*case arrow*)
    ...
    assert (typing nil (e_ann (e_abs e) (t_arrow A1 B1)) (t_arrow A1 B1)); eauto.
  - (*mergl*)
    inverts Val.
    apply inv_merge in Typ.
    destruct Typ as [B1 [B2 [Typ1 [Typ2 Sub]]]].
...
Lemma value_dir_bot_false : forall p E,
  value p ->
...
Proof.
  ...
  eapply sub_transitivity in Sub; eauto.
  inverts* Val. inverts* Sub. inverts* Sub.
Qed.

(*

tred_progress corresponds to Theorem 2 (Casting Progress)
in paper.

*)

Lemma tred_progress : forall v,
value v -> forall A, typing nil v A -> exists v', tred v A v'.
Proof.
  ...
  - (* case int *)
    ...
    inverts Typ.
    forwards*: value_dir_bot_false H H0. inverts H1.
  ...
  - (* case arrow *)
    ...
    destruct Typ as [Typ Sub]. inverts Sub.
    inverts Typ.
    forwards*: value_dir_bot_false H0. inverts H2.
    clear IHA0_1 IHA0_2.
    apply inv_abs_sub'' in Typ.
    destruct Typ as [Typ Sub].
    eauto.
  ...
  - (* case intersection *)
    ...
    (*union type*)
    assert (Val: value (e_merg v1 v2)); auto.
...
Proof.
  ...
  - inverts H. inverts* STEP. inverts H2.
  - inverts* STEP. inverts* H. inverts* H.
Qed.
...
(*
getInTypeInv corresponds to Lemma 3 (Applicative dispatch compatibility)
in paper.
*)
...
Lemma value_ptype_sub_dir_type : forall w A,
  value w ->
  typing nil w A ->
  forall B, ptyp w B ->
  B <: A.
...
preservation corresponds to Theorem 5 (Type Preservation)
in paper.
...
Proof.
  ...
  - (*anno*)
    ...
    inductions Typ. apply sub_and in H.
    destruct H as [Sub1 Sub2].
    eapply typ_merg. eauto. eauto.
    eauto.
  - (* app *)
     inverts* Red.
     (* beta *)
   ...
   + (*case dynsemantics*)
      ...
      forwards*: value_ptype_sub_dir_type H.
      clear H4.
   ...
   + (* un abs *)
    apply inv_anno in Typ1.
    destruct Typ1 as [Typ1 Sub].
    inverts Sub.
    apply typ_ann in Typ1.
    assert (typing []
    (e_app (e_ann (e_abs e) (t_arrow A1 B1))
       (e_ann (e_abs p) A1)) B1); eauto.
  - (*switch*)
    inverts* Red.
    (* when lies in first branch *)
    pick_fresh y.
    forwards* TEMP1: H y.
    lets TEMP2: tred_preservation H10 H11.
...
Lemma exists_intype_value : forall p,
  value p -> exists A, getInType p A.
Proof.
...
dynSemanticsProgress corresponds to Lemma 4 (Applicative dispatch progress)
in paper.
...
Proof.
  ...
  forwards* TEMP2: exists_intype_value PVAL1.
  forwards* TEMP3: exists_intype_value PVAL2.
  ...
  forwards*: value_ptype_sub_dir_type PTYP1.
...
progress corresponds to Theorem 6 (Progress) in paper.

*)

Lemma value_dec : forall e, lc_exp e -> value e \/ not (value e).
Proof.
  induction e; unfold not; intros; auto;
  try solve [right; introv NVALUE; inverts NVALUE].
  - inverts H.
    destruct~ IHe as [VAL | NVAL].
    right. introv NVALUE. inverts* NVALUE. inverts VAL.
    destruct e;
    try solve [right; introv VAL; inverts VAL].
    destruct t; auto;
    try solve [right; introv VAL; inverts VAL].
  - inverts H.
    destruct IHe1 as [VAL1 | VAL1]; 
    destruct IHe2 as [VAL2 | VAL2]; auto;
    try solve [right; introv VAL; inverts* VAL].
Qed.
...
(* Inductive wvalue : exp -> Prop :=    (* defn value *)
 | wval_val : forall v,
    value v ->
    wvalue v 
 | wval_abs_unann : forall (e:exp) (A B:typ),
     lc_exp (e_abs e) ->
     wvalue (e_abs e).

#[export]
Hint Constructors wvalue : core.

Lemma wvalue_value : forall w,
wvalue w -> value w \/ (exists e, w = (e_abs e)).
Proof.
 induction 1; eauto.
Qed.

Lemma value_wvalue : forall v,
value v -> wvalue v.
Proof.
 auto.
Qed. *)
...
inductions Typ; eauto.
 ...
 - forwards*: IHTyp. destruct~ H.
  ...
  + (*case step-ann*)
    ...
    forwards* DEC: value_dec (e_ann e A).
 ...
 - right. forwards* TEMP1: IHTyp1.
   ...
   + forwards* TEMP2: IHTyp2.
     ...
     *  inverts* H.
        ...
        apply tred_progress in Typ3; auto.
        destruct Typ3 as [v' Tred]. eauto.
        (*case merge*)
        forwards* : dynSemanticsProgress Typ1 Typ2.
     * destruct H0 as [e'].
       eauto.
  + destruct H.
...
(*case typ-merge*)
 ...
 - (*switch*)
   ...
   destruct IHTyp; auto.
   (*value e*)
   forwards*: check_or_typ H3.
   destruct H4.
   forwards*: tred_progress H3.
   destruct H5 as [p' H5].
...
Lemma progress1 : forall e T,
typing nil e T -> (value e) \/ (exists e', e ~~> e')
\/ (exists e', e = (e_abs e')).
Proof.
introv Typ. gen T.
induction e; eauto; intros.
 (*case var*)
 - apply typing_regular in Typ.
   destruct Typ as [_ lc]. inverts lc.
 - (*var*)
   inductions Typ. inverts H0.
   forwards*: IHTyp.
   forwards*: IHTyp1.
 (*case anno*)
 - apply inv_anno in Typ.
